...
export const UpdateStudyGroupPage = () => {
    const { id } = useParams()
    ...
    // Загружаем список существующих persons
        ...
        const fetchStudyGroup = async () => {
            ...
            try {
                ...
                setFormData({
                    name: data.name || '',
                    coordinates: {
                        x: data.coordinates?.x?.toString() || '',
                        y: data.coordinates?.y?.toString() || ''
                    },
                    studentsCount: data.studentsCount?.toString() || '',
                    formOfEducation: data.formOfEducation || '',
                    semesterEnum: data.semesterEnum || '',
                    groupAdmin: data.groupAdmin?.id?.toString() || '' // Сохраняем ID админа
                })
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                groupAdmin: formData.groupAdmin ? formData.groupAdmin : null // Отправляем только ID
            }

            await studyGroupsAPI.update(id, payload)
            navigate('/studygroups')
        } catch (err) {
            setError('Ошибка при обновлении StudyGroup: ' + (err.response?.data?.message || err.message))
            console.error(err)
        } finally {
            setLoading(false)
        }
            ...
            <div className="fade-in">
                <div className="card">
                    <div style={{ textAlign: 'center', padding: '3rem' }}>
                        ...
                        <p className="text-muted">Loading group data...</p>
                    </div>
                </div>
            </div>
        )
        ...
        <div className="fade-in">
            <div className="card">
                <div className="card-header">
                    <h1>✏️ Update StudyGroup</h1>
                    <p className="text-muted">Edit the details of study group #{id}</p>
                </div>
